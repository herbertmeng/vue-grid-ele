...
const defaults = {
  ...
  selection: {
    ...
    realWidth: 48,
    order: '',
    className: 'v-table-column--selection'
  },
  expand: {
    width: 48,
    minWidth: 48,
...
const forced = {
  ...
  expand: {
    ...
    renderCell: function(h, { row, store }, proxy) {
      ...
      return <div class={ 'v-table__expand-icon ' + (expanded ? 'v-table__expand-icon--expanded' : '') }
                  on-click={ () => proxy.handleExpandClick(row) }>
        <i class='v-icon v-icon-arrow-right'></i>
      </div>;
    },
    sortable: false,
    resizable: false,
    className: 'v-table__expand-column'
  }
};
  ...
  created() {
    ...
    column.renderCell = function(h, data) {
      ...
      return _self.showOverflowTooltip || _self.showTooltipWhenOverflow
        ? <div class="cell v-tooltip" style={'width:' + (data.column.realWidth || data.column.width) + 'px'}>{ renderCell(h, data) }</div>
