...
export default {
        ...
        <tbody>
          {
            this._l(this.data, (row, $index) =>
              [<tr
                ...
                ? (<tr>
                    <td colspan={ this.columns.length } class="v-table__expanded-cell">
              ...
              <v-tooltip effect={ this.table.tooltipEffect } placement="top" ref="tooltip" content={ this.tooltipContent }></v-tooltip>
            )
          }
        </tbody>
      </table>
  ...
  watch: {
    'store.states.hoverRow'(newVal, oldVal) {
      ...
      const el = this.$el;
      if (!el) return;
      const rows = el.querySelectorAll('tbody > tr.v-table__row');
  ...
  methods: {
    ...
    getRowClass(row, index) {
      const classes = ['v-table__row'];

      if (this.stripe && index % 2 === 1) {
        classes.push('v-table__row--striped');
      }
    ...
    handleCellMouseEnter(event, row) {
      ...
      if (hasClass(cellChild, 'v-tooltip') && cellChild.scrollWidth > cellChild.offsetWidth) {
